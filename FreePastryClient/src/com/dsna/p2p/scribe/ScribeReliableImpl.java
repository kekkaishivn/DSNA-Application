...
/**
 * @(#) ReliableScribeImpl.java: Extend the ScribleImpl provided implementation of Scribe.
 * Implementing caching mechanism to allow reliable publish/subscribe
 *
 * @version $Id: ScribeImpl.java 4495 2008-09-26 14:48:02Z jeffh $
 * @author 
 */
public class ScribeReliableImpl implements Scribe, MaintainableScribe, Application, Observer {
  // Special Log Levels
   ...
   */
  private DSNAScribeMaintenancePolicy maintenancePolicy;
  ...
  /**
   * Topics that we are the implicit root, i.e no one subscribe to it at the moment
   */
  public Set<Topic> implicitRoots = new HashSet<Topic>();
  
  /**
   * This contains a mapping of topic - > its replica pulling recently
   */
  public HashMap<Topic, ArrayList<DSNAScribeCollectionContent>> replicaManager = new HashMap<Topic, ArrayList<DSNAScribeCollectionContent>>();
  
  /**
   * Topics that we are currently subscribing to
   */
  public Set<Topic> subscribes = new HashSet<Topic>();
  
  /**
   * This contains a mapping of topic - > its caching published msg for which the local node
   * is topic's root
   */
  HashMap<Topic, ArrayList<ScribeContent>> publishedCaches = new HashMap<Topic, ArrayList<ScribeContent>>();
   ...
   */
  public ScribeReliableImpl(Node node, String instance) {
   ...
   * @param instance The unique instance name of this Scribe
   */
  public ScribeReliableImpl(Node node, ScribePolicy policy, String instance) {
    this(node, policy, instance, new DSNAScribeMaintenancePolicy.DSNAReliableScribeMaintenancePolicy(node.getEnvironment()));
  }
   ...
   * @param instance The unique instance name of this Scribe
   */
  public ScribeReliableImpl(Node node, ScribePolicy policy, String instance, DSNAScribeMaintenancePolicy maintenancePolicy) {
    this.environment = node.getEnvironment();
    this.node = node;
    logger = environment.getLogManager().getLogger(ScribeReliableImpl.class, instance);
      ...
      public Message deserialize(InputBuffer buf, short type, int priority,
        ...
        try {
          switch(type) {
            ...
            // new in replicate caching version:
            case ReplicateContentsMessage.TYPE:
            	return ReplicateContentsMessage.build(buf, endpoint, contentDeserializer);
            case ReplicatePushReqMessage.TYPE:
            	return ReplicatePushReqMessage.build(buf, endpoint, contentDeserializer);
            case ReplicatePushACKMessage.TYPE:
            	return ReplicatePushACKMessage.build(buf, endpoint, contentDeserializer);
            case ReplicatePullReqMessage.TYPE:
            	return ReplicatePullReqMessage.build(buf, endpoint, contentDeserializer);
          }
        } catch (IOException e) {
          if (logger.level <= Logger.SEVERE) logger.log("Exception in deserializer in "+ScribeReliableImpl.this.endpoint.toString()+":"+ScribeReliableImpl.this.instance+" "+contentDeserializer+" "+e);
          throw e;
        }
...
/*  	  		try {
  ...
  protected void doSubscribe(Collection<Topic> theTopics, ScribeMultiClient client, RawScribeContent content, NodeHandle hint) {
    ...
    if (client != null) {
      if (!alreadySubscribed.isEmpty())	{
      	...
      	System.out.println("Process some already subscribed");
	      if (client instanceof ScribeReliableMultiClient)
		      for(Topic theTopic:alreadySubscribed)	{
		      	System.out.println("Push the contents");
		      	ArrayList<ScribeContent> cache = publishedCaches.get(theTopic);
		      	ScribeReliableMultiClient rclient = (ScribeReliableMultiClient)client;
		      	if(cache!=null)
		      		rclient.contentsReceived(cache, theTopic);
		      }
      }
    }
  ...
  private DSNAScribeCollectionContent getMissingContents(Topic topic)	{
  	
  	DSNAScribeCollectionContent theMissingContents = null;
  	List<ScribeContent> cache = publishedCaches.get(topic);
  	long latestLocalSeq = topic.getSeq();
  	
  	// If cache null or subscriber ignore missing contents, return null
  	// Otherwise, return missing contents based on latestLocalSeq in subscribe msg
  	if (cache==null || cache.isEmpty() || latestLocalSeq == Topic.SEQ_IGNORE) 
  		return null;  	
  	
  	int lastElementIndex = cache.size()-1;
  	long latestSeq = ((DSNAScribeContent)cache.get(lastElementIndex)).seq;
  	if (latestLocalSeq == Topic.SEQ_CACHE_NULL)	{
  		theMissingContents = new DSNAScribeCollectionContent(localHandle, cache);
...
//           
      ...
      if (logger.level <= Logger.FINER) logger.log("Received publish request message with data " +
    	...
    	System.out.println("Received publish request message with data " +
          prMessage.getContent() + " for topic " + prMessage.getTopic());
    	
    	// Should cache the message here for both explicit root & implicit root case
    	System.out.println( "Topic "+prMessage.getTopic()+" isCaching is +"+prMessage.getTopic().isCaching());
      if (prMessage.getTopic().isCaching())	{
      	final Topic theTopic = prMessage.getTopic();
      	final DSNAScribeContent theContent = (DSNAScribeContent)prMessage.getContent();
      	
      	if (publishedCaches.get(theTopic) == null)	{
        	Runnable cachingWorker = new Runnable() {
  	        public void run() {    
  	        	cachePublishMessageAsRoot(theTopic, theContent);
      ...
      if (manager == null) {
      	implicitRoots.add(prMessage.getTopic());
      ...
      if ((manager != null) && ((manager.getParent() == null) || (manager.getParent().equals(pMessage.getSource())))) {
        ...
        // Caching the publish event if we are not root of this topic
        // Otherwise, we double caching a msg
        if(pMessage.getTopic().isCaching() && !this.isRoot(pMessage.getTopic()))	{
        	cachePublishMessage(pMessage.getTopic(),pMessage.getContent());
        }
    ...
    } else if (message instanceof ReplicateContentsMessage) {
    	ReplicateContentsMessage rcMsg = (ReplicateContentsMessage) message;
      if (logger.level <= Logger.FINE)
        logger.log("Received replicate contents message from root " + rcMsg.getSource()+ " for topic " + rcMsg.getTopic());
      ...
      if (theTopic.isCaching())	{
      	...
      	} else if (!isAlreadySubscribed(theTopic))
      					cachePublishMessage(rcMsg.getTopic(), rcMsg.getContent());

      }
      	

    } else if (message instanceof ReplicatePushReqMessage) {
    	ReplicatePushReqMessage rrMsg = (ReplicatePushReqMessage) message;
      if (logger.level <= Logger.FINE)
        logger.log("Received replicate request from root " + rrMsg.getSource()+ " for topic " + rrMsg.getTopic());
      
      System.out.println("Received replicate request from root " + rrMsg.getSource()+ " for topic " + rrMsg.getTopic());
      // Sending back the local latest seq in replicate ack message
      // In case there is no cache before, sending TAKE_ALL 
      Topic theTopic = rrMsg.getTopic();
      long localLatestSeq = this.getLatestSeqInCache(theTopic);
      theTopic.setSeq(localLatestSeq);
      endpoint.route(null, new ReplicatePushACKMessage(localHandle, theTopic), rrMsg.getSource());
      
    } else if (message instanceof ReplicatePushACKMessage) {
    	ReplicatePushACKMessage raMsg = (ReplicatePushACKMessage) message;
      if (logger.level <= Logger.FINE)
        logger.log("Received replicate ack from replicated node " + raMsg.getSource()+ " for topic " + raMsg.getTopic());
      
      System.out.println("Received replicate ack from replicated node " + raMsg.getSource()+ " for topic " + raMsg.getTopic());
      // Sending missing contents to the replicate 
      DSNAScribeCollectionContent missingContents = this.getMissingContents(raMsg.getTopic());
      if (missingContents != null && missingContents.getSize()>0)	{
      	System.out.println("Route message to : "+raMsg.getSource());
      	this.endpoint.route(null, new ReplicateContentsMessage(this.localHandle, raMsg.getTopic(), missingContents), raMsg.getSource());
      }
    } else if (message instanceof ReplicatePullReqMessage) {
    	ReplicatePullReqMessage rprMsg = (ReplicatePullReqMessage) message;
      if (logger.level <= Logger.FINE)
        logger.log("Received replicate pull request from root node " + rprMsg.getSource()+ " for topic " + rprMsg.getTopic());
      
      System.out.println("Received replicate pull request from root node " + rprMsg.getSource()+ " for topic " + rprMsg.getTopic());
      // Sending missing contents to the replicate 
      DSNAScribeCollectionContent missingContents = this.getMissingContents(rprMsg.getTopic());
      System.out.println(missingContents);
      if (missingContents != null && missingContents.getSize()>0)	{
      	System.out.println("Route message to : "+rprMsg.getSource());
      	endpoint.route(null, new ReplicateContentsMessage(localHandle, rprMsg.getTopic(), missingContents), rprMsg.getSource());
      }
    } else {
  ...
  private void cachePublishMessage(Topic topic, ScribeContent content) {
  	synchronized (publishedCaches)	{
			// Local caching
	  	System.out.println("Caching some content "+content+" In the topic + "+topic);
			ArrayList<ScribeContent> theCache = publishedCaches.get(topic);
			
			if (theCache == null)	{
				theCache = new ArrayList<ScribeContent>();
				publishedCaches.put(topic, theCache);
			}
			
			if(content instanceof DSNAScribeCollectionContent)	{
				Collection<ScribeContent> contentsList = ((DSNAScribeCollectionContent)content).contents;
				for (ScribeContent theContent : contentsList)
					theCache.add(theContent);
			} else	{
  ...
  private void cachePublishMessageAsRoot(Topic topic, ScribeContent content)	{
  	if (!isRoot(topic)) return;
  	synchronized (publishedCaches)	{
  		ArrayList<ScribeContent> theCache = publishedCaches.get(topic);
  		if (theCache == null)	{
  			if (!pullReplica(topic))	{
  				theCache = new ArrayList<ScribeContent>();
  				publishedCaches.put(topic, theCache);  				
  			} else	{
  ...
  void asynchronousReplicateCache(Topic topic)	{
  	System.out.println("Doing asynchronous replication");
		// request asyncronous repilicate for the topic we are root
		if (isRoot(topic))	{
      NodeHandleSet replicationSet = endpoint.replicaSet(topic.getId(), REPLICATE_FACTOR);
      if (replicationSet!=null)	{
      	...
      	for (int i=0; i<replicationSet.size(); i++)	{
      		NodeHandle theNode = replicationSet.getHandle(i);
      		endpoint.route(null, new ReplicatePushReqMessage(localHandle, topic), theNode);
      	}
      }
		}  	
  }
  
  private long getLatestSeqInCache(Topic topic)	{
  	long result = Topic.SEQ_CACHE_NULL;
  	synchronized (publishedCaches)	{
	  	ArrayList<ScribeContent> cache = this.publishedCaches.get(topic);
	  	if (cache!=null && !cache.isEmpty())	{
	  		result = ((DSNAScribeContent)cache.get(cache.size()-1)).seq;
	  	}
  	}
  	return result;
  }
  
  private void printCachedPublishMessage(Topic topic) {
		// TODO Auto-generated method stub
		ArrayList<ScribeContent> theCache = this.publishedCaches.get(topic);
		System.out.println("Print out the cache "+theCache+" For topic"+topic);
		if(theCache!=null)	{
			for(ScribeContent c : theCache)
				System.out.println(c.toString());
		}
	}
  
  public boolean isAlreadySubscribed(Topic topic)	{
    synchronized(topicManagers) {
      TopicManager manager = topicManagers.get(topic);
      return manager != null;
    }  	
...
//      o.deleteObserver(this); // this should be done automatically when the maintenance policy gets rid of the parents/children
  ...
  public void update(NodeHandle handle, boolean joined) {
    ...
    // Dealing with the case of implicit root
    // If found new root, passing the burden to it
/*    System.out.println("Dealing with :"+implicitRoots);
    for (Topic implicitTopic : implicitRoots)	{
    	if (!isRoot(implicitTopic) && implicitTopic.isCaching())	{
      	System.out.println("Passing cache to new implicit root");
      	DSNAScribeCollectionContent theCachingContents = getMissingContents(implicitTopic);
      	if (theCachingContents!=null)	{
      		NodeHandle newRoot = getRoot(implicitTopic);
      		implicitTopic.setSeq(Topic.SEQ_CACHE_NULL);
      		if (newRoot!=null)
      			endpoint.route(null, new ReplicateContentsMessage(localHandle, implicitTopic, theCachingContents), newRoot);     
      	}
    	}
    }*/    
...
//          sendSubscribe(topic, null, null, null);
          ...
          //Passing topic's cache to new root 
          if (topic.isCaching())	{
          	System.out.println("Passing cache to new root");
          	DSNAScribeCollectionContent theCachingContents = getMissingContents(topic);
          	if (theCachingContents!=null)	{
          		NodeHandle newRoot = getRoot(topic);
          		topic.setSeq(Topic.SEQ_CACHE_NULL);
          		if (newRoot!=null)
          			endpoint.route(null, new ReplicateContentsMessage(localHandle, topic, theCachingContents), newRoot);
          	}
          }
